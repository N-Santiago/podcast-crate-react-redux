...
export const searchPodcastQuery = (query) => {
    return (dispatch) => {
        return fetch(
            `https://listen-api.listennotes.com/api/v2/search?q=${query}&sort_by_date=0&type=episode&offset=0&len_min=10&len_max=30&genre_ids=68%2C82&published_before=1580172454000&published_after=0&only_in=title%2Cdescription&language=English&safe_mode=0`,
            {
                headers: {
                    'X-ListenAPI-Key': '521822f39f8d4140bd5ab8dcdae30b7f',
                },
            }
        )
            .then((response) => response.json())
